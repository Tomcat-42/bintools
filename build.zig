...
pub fn build(b: *std.Build) !void {
    ...
    bintools_mod.addAssemblyFile(b.path("src/add.s"));

    // Targets
    const bintools = b.addExecutable(.{
        .name = "bintools",
        .root_module = bintools_mod,
    });

    // Run
    const run_cmd = b.addRunArtifact(bintools);
    ...
    targets.appendAssumeCapacity(bintools);
    const compile_commands_step = CompileCommands.createStep(b, "compile_commands", try targets.toOwnedSlice());
    bintools.step.dependOn(compile_commands_step);
...
// Very sussy code ahead ඞ ඞඞඞඞඞඞඞඞඞඞඞඞඞඞ…
const CompileCommands = struct {
    ...
    pub fn createStep(b: *std.Build, name: []const u8, targets: []*std.Build.Step.Compile) *Step {
    ...
    fn makeCdb(step: *std.Build.Step, make_options: std.Build.Step.MakeOptions) anyerror!void {
        ...
        var file = try cwd.createFile(b.fmt("{any}/{s}", .{ b.cache_root, "compile_commands.json" }), .{});
        defer file.close();
